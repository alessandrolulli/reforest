...
import org.apache.spark.ml.tree.impl.{RandomForestRunner, RandomForest => NewRandomForest}
...
/**
  ...
  */
private class RandomForestInstrumented (
  ...
  require(RandomForestInstrumented.supportedFeatureSubsetStrategies.contains(featureSubsetStrategy)
  ...
  def run(input: RDD[LabeledPoint]): RandomForestModel = {
    val trees: Array[NewDTModel] = RandomForestRunner.run(input.map(_.asML), strategy, numTrees,
...
@Since("1.2.0")
object RandomForestInstrumented extends Serializable with Logging {
  ...
  def trainClassifier(
    ...
    val rf = new RandomForestInstrumented(strategy, numTrees, featureSubsetStrategy, seed)
    rf.run(input)
  }
