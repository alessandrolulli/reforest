...
/**
  * Code from: https://gist.github.com/non/29f8d66036afca402f96
  */
class Float16(val raw: Short) extends Serializable { lhs =>
  ...
  def /(rhs: Float16): Float16 =
    Float16.fromFloat(lhs.toFloat / rhs.toFloat)
  def **(rhs: Int): Float16 =
    Float16.fromFloat(Math.pow(lhs.toFloat, rhs).toFloat)
  ...
  def fromFloat(n: Float): Float16 = {
    ...
    def createFinite(e: Int, x: Float): Float16 = {
      ...
      if (e == -14 && m == 0) {
        ...
        else new Float16((0x8000 | Math.round(x * 1024)).toShort)
      } else if (e == 15) {
...
//        val x1 = n1.toFloat
//
//        val nr = new Float16(Random.nextInt.toShort)
//        val xr = nr.toFloat
//
//        if (JFloat.compare(x0, x1) != 0) {
//          fail += start
//        } else if (JFloat.compare(n0.signum, Math.signum(x0)) != 0) {
//          fail += start
